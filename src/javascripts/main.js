...
function searchMovies(event) {
    ...
    let count = 0
    for (let m of movies) {
        if (m.title.toUpperCase().indexOf(input.toUpperCase()) == -1) { // if no matches found add no display
            document.querySelector(`#m${m.id}`).classList.add('d-none')
        } else {
            document.querySelector(`#m${m.id}`).classList.remove('d-none')//if matched, remove no display
            count++
        }
    }
...
//make sure submissions go through the funciton
